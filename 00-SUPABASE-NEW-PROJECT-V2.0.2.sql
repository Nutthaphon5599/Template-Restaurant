...
-- SUPABASE NEW PROJECT - COMPLETE INSTALLATION (ONE FILE)
-- ============================================================
-- ใช้สำหรับ Supabase Project ใหม่เท่านั้น
-- วิธีใช้: SQL Editor > New query > วางโค้ดทั้งหมด > Run
-- ไฟล์นี้รวมโครงสร้างฐานข้อมูล, RLS, Storage, VAT,
-- Currency, Theme, Setup Wizard และ restaurant_tables แล้ว
-- ไม่ต้องรัน SQL ไฟล์อื่นเพิ่ม
-- ============================================================
...
create table if not exists public.restaurant_settings (
  ...
  restaurant_name_lo text not null default 'ຊື່ຮ້ານອາຫານ',
  restaurant_name_th text not null default 'ชื่อร้านอาหาร',
  restaurant_name_en text not null default 'Restaurant Template',
  phone_display text not null default '020 0000 0000',
  phone_intl text not null default '8562000000000',
  whatsapp_intl text not null default '8562000000000',
  facebook_name text default 'Your Facebook Page',
  address_lo text,
  address_th text,
  address_en text,
  maps_url text,
...
-- ===== INCLUDED V8.2 FIXED VAT UPGRADE =====
-- ============================================================
-- Restaurant POS V8.2 - SAFE UPGRADE (FIXED)
-- Run ONCE in Supabase SQL Editor using the postgres role.
-- Keeps existing menu, order, table and sales data.
-- ============================================================
...
alter table public.restaurant_settings add column if not exists vat_mode text;
alter table public.restaurant_settings add column if not exists vat_rate numeric(5,2);
alter table public.restaurant_settings add column if not exists updated_at timestamptz;

update public.restaurant_settings
set vat_mode=coalesce(vat_mode,'inclusive'),
...
do $$
begin
  if not exists (select 1 from pg_constraint where conrelid='public.restaurant_settings'::regclass and conname='restaurant_settings_single_row_check') then
    alter table public.restaurant_settings add constraint restaurant_settings_single_row_check check (id=1);
  end if;
  if not exists (select 1 from pg_constraint where conrelid='public.restaurant_settings'::regclass and conname='restaurant_settings_vat_mode_check') then
    alter table public.restaurant_settings add constraint restaurant_settings_vat_mode_check check (vat_mode in ('inclusive','exclusive'));
  end if;
  if not exists (select 1 from pg_constraint where conrelid='public.restaurant_settings'::regclass and conname='restaurant_settings_vat_rate_check') then
    alter table public.restaurant_settings add constraint restaurant_settings_vat_rate_check check (vat_rate>=0 and vat_rate<=100);
  end if;
end $$;
...
create policy "authenticated can read restaurant settings" on public.restaurant_settings for select to authenticated using(true);
drop policy if exists "authenticated can update restaurant settings" on public.restaurant_settings;
create policy "authenticated can update restaurant settings" on public.restaurant_settings for all to authenticated using(true) with check(true);
grant select,insert,update on public.restaurant_settings to authenticated;

do $$
begin
  if to_regclass('public.orders') is not null then
    alter table public.orders add column if not exists vat_mode text;
    update public.orders set vat_mode='inclusive' where vat_mode is null;
    alter table public.orders alter column vat_mode set default 'inclusive';
    if not exists (select 1 from pg_constraint where conrelid='public.orders'::regclass and conname='orders_vat_mode_check') then
      alter table public.orders add constraint orders_vat_mode_check check(vat_mode in ('inclusive','exclusive'));
    end if;
  end if;
end $$;

commit;
select 'V8.2 fixed upgrade complete' as status,vat_mode,vat_rate from public.restaurant_settings where id=1;


-- Restaurant Template V2.0.0 migration
-- Run once after V1 installation. Safe to re-run.
begin;
create extension if not exists pgcrypto;
alter table public.restaurant_settings add column if not exists default_language text default 'lo';
alter table public.restaurant_settings add column if not exists currency_code text default 'LAK';
alter table public.restaurant_settings add column if not exists theme_primary text default '#0b5d3b';
alter table public.restaurant_settings add column if not exists theme_accent text default '#d8ad56';
alter table public.restaurant_settings add column if not exists setup_completed boolean default false;
alter table public.restaurant_settings add column if not exists vat_mode text default 'inclusive';
alter table public.restaurant_settings add column if not exists vat_rate numeric(5,2) default 10;
create table if not exists public.restaurant_tables(
 id uuid primary key default gen_random_uuid(), table_number integer not null unique, label text, capacity integer not null default 4,
 active boolean not null default true, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
alter table public.restaurant_tables enable row level security;
drop policy if exists "Public read restaurant tables" on public.restaurant_tables;
create policy "Public read restaurant tables" on public.restaurant_tables for select to anon,authenticated using(true);
drop policy if exists "Admins manage restaurant tables" on public.restaurant_tables;
create policy "Admins manage restaurant tables" on public.restaurant_tables for all to authenticated using(public.is_active_admin()) with check(public.is_active_admin());
grant select on public.restaurant_tables to anon,authenticated; grant insert,update,delete on public.restaurant_tables to authenticated;
insert into public.restaurant_tables(table_number,label,capacity,active)
select n,'ໂຕະ '||n,4,true from generate_series(1,20)n on conflict(table_number) do nothing;
update public.restaurant_tables set label='ໂຕະ '||table_number where label is null or label like '%ຕູບ%';
commit;
select 'Restaurant Template V2.0.2 new project ready' as status,(select count(*) from public.restaurant_tables where active) as active_tables;
